...
def CheckSymbol(Elements):
    ...
    if (
        (len(Elements) > 1)
        and ([Elements[-1], None, None, "Variable"] not in SymbolTable)
        and (Elements[-1] != "CLA")
        and (Elements[-2] not in ["BRP", "BRN", "BRZ"])
        and (Elements[-1][:2] != "='")
        and (Elements[-1][:3] != "REG")
        and (not Elements[-1].isnumeric())
    ):
        return True
    else:
        return False
...
file = open("C:\\Users\\shabb\\OneDrive\\Desktop\\Hatim\\SPCC-Practicals\\Exp8\\assembly.txt", "r")
...
for line in file:
    ...
    if line[:2] != "//":
        if line.strip() != "START":
            print(
                "STARTError : 'START' statement is missing. "
                + "( Line "
...
for line in file:
    ...
    if line[:2] != "//":
        ...
        if (len(Elements) >= 3) and (Elements[0] in Opcodes):
            print(
                "TooManyOperandsError : Too many operands used for the '"
                + Elements[0]
                + "' assembly opcode. "
                + "( Line "
                + str(location_counter)
        ...
        if (len(Elements) == 1) and (
            ...
            print(
                "LessOperandsError : Less operands used for the '"
                + Elements[0]
                + "' assembly opcode. "
                + "( Line "
                + str(location_counter)
...
if end_found is False:
    print(
        "ENDError : 'END' statement is missing."
        + "( Line "
        + str(location_counter)
        + " )"
    )
        ...
        # If the CLA opcode has a Label before it
            ...
            # If there is no Label
                ...
                if CheckLiteral(Elements[1]):
                    for i in range(len(LiteralTable)):
                        if LiteralTable[i][0] == Elements[1]:
                            AssemblyCode.append(
                                s + "00 00 " + str(LiteralTable[i][1]).rjust(2, "0")
                            )
                ...
                # Dealing with Lables (BRP, BRZ, BRN)
                    ...
                    for i in range(len(SymbolTable)):
                        if SymbolTable[i][0] == Elements[1]:
                            AssemblyCode.append(
                                s + str(SymbolTable[i][1]).rjust(2, "0") + " 00 00"
                            )
                            print(str(SymbolTable[i][1]).rjust(2, "0") + " 00 00")
                # Dealing with Registers
                elif Elements[1][:3] == "REG":
                    AssemblyCode.append(
                        s + "00 " + Elements[1][-1].rjust(2, "0") + " 00"
                    )
                ...
                # Dealing with Variables
                    ...
                    for i in range(len(SymbolTable)):
                        if SymbolTable[i][0] == Elements[1]:
                            AssemblyCode.append(
                                s + "00 00 " + str(SymbolTable[i][1]).rjust(2, "0")
                            )
            ...
            # If the instruction conatins a Label
                ...
                # Dealing with Registers
                    ...
                    AssemblyCode.append(
                        s + "00 " + Elements[2][-1].rjust(2, "0") + " 00"
                    )
